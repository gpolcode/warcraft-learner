...
description: Generate or refresh a warcraft-learner spec's rulebook.json clean-slate from current sources, through the `rulebook-author` subagent. Load whenever creating or refreshing rulebooks for one spec, several, or all of them, at a new patch or tier, or when onboarding a spec.
---
...
**Division of labor:** the **orchestrator** (you) does everything mechanical - source fetching, stripping, spell-id grounding, schema validation, publishing. The **`rulebook-author` subagent** (`.claude/agents/rulebook-author.md`) turns the scratchpad files you prepared into one spec's rulebook and reads nothing else.
...
4. **Validate and write (orchestrator).** Each dispatch overwrites `frontend/public/data/specs/{spec}/rulebook.json` (pretty-printed, `spec` = folder key, required `spec_icon` set). Then run the full validation pass from reference.md over **every file on disk for the selected specs** - a killed run leaves a plausible file behind whose failure notice says nothing about it. Send a single-rule defect back to the spec's live dispatch - fixing one rule in place is far cheaper than a cold re-run; reserve a fresh dispatch for broad misses.
