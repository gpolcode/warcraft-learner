...
5. **Publish.** `public/data/specs/**` is gitignored on code branches; rulebooks live on `gh-pages` at `data/specs/{spec}/rulebook.json`. Publish via a `gh-pages` worktree and a PR with base `gh-pages`; the hourly ingest rebuilds that spec's benches over its next passes (see CLAUDE.md).
