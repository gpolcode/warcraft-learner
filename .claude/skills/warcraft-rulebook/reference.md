...
`.claude/agents/rulebook-author.md`.
...
**A guide page sometimes carries another spec's prose spliced into it.** One Midnight-era
Devastation Evoker rotation page embeds Fire Mage lines (Combustion, Hot Streak, Fire Blast)
complete with their `[spell=...]` codes, so the ids survive extraction and reach the ability
table as real, name-verified rows. The name gate cannot catch this: the ids are genuine, just
another class's. Skim the stripped guide for abilities the spec does not own before dispatch,
and drop those rows.
