...
2. Every cooldown entry **must include a `spell_id`**. Use your knowledge of Retail WoW spell IDs to fill this in - prioritize the Active Ability Cast ID rather than a passive Aura, Classic WoW ID, or Talent Node ID. You can verify spell IDs at `wowhead.com/spell=<id>`. If you are genuinely unsure of the ID, make your best guess and record the uncertainty in the separate `id_note` field. **Never** put best-guess or "verify this ID" notes in `usage_rule` - that field is shown to users as coaching copy, so it must stay clean; `id_note` is internal and never displayed.
3. Extract all **personal defensive cooldowns** for this spec (abilities that reduce damage taken, grant immunity, or provide significant self-healing, with a cooldown ≥ 15 s). This is **required** - the `defensives` array must always be present and populated. Every entry needs a `spell_id`. **Only include abilities that belong to this spec** - do not include abilities from other specializations of the same class (e.g. do not include Arms-only abilities in a Fury Warrior rulebook).
...
5. **Talent-gated abilities**: If an ability requires a specific talent that is not taken by all builds of this spec (e.g. it is only used by one hero talent tree or is a notable talent choice), set `"talent_gated": true` on that entry. This prevents false "never used" alerts for players who did not take that talent.
6. Output **only** the raw JSON object. Do NOT wrap the output in markdown code fences (e.g., do not use ```json). No explanation, no preamble. The first character of your reply must be `{` and the last must be `}`.
