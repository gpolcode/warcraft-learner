...
| `Unparsed expression`, `Unparsed line`, `Missing list` | Extending the parser or the reader; the profile is right, the reader is behind. A list cycle is walked once and is no gap. |
...
The encounter stamp is `<INGEST_VERSION>:<key>`, the key being the first 16 hex digits of the SHA-256 over each resolved line's action and printed gates plus the spec's owned records after exclusions; a spec with no profile stamps on the version alone. A SimulationCraft change the rules can see re-benches the spec's encounters when its turn in the run order comes, which spreads the work evenly; one they cannot see re-benches nothing. A derivation change is a code change and bumps `INGEST_VERSION` (warcraft-change), the one knob the run order reads.
