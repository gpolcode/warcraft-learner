...
2. Each `*FeatureService` reads its prepared bench via its `*DataSource` (the tailored file in prod, or the live `*TransformService` in the development and ingest environments) and fetches the player's own log (`Casts`/`Buffs`/`DamageDone`/`DamageTaken`) via the cached `WclApiService`, then computes its slice with its colocated pure functions:
   - **Rotation** (`rotation.service.ts`) - per offensive cooldown: lost casts (`expected` from top-parse `uses_per_min`; the lost-cast and first-cast checks are gated on a use-share majority - `used_sample_count / sample_count >= MIN_USE_SHARE_FRAC` - so a situational cd most top parses skip is never flagged as "unused"), bloodlust alignment (gated by measured `bl_pct >= 50`, not the rulebook flag), first-cast delay (>2σ), held-past-reset (>2σ above `avg_gap_s`, skipped when null), hold suggestions (a majority of top parsers hold past the prior cast + cooldown; flagged when the player's own prior-relative gap falls below the consensus band), cast efficiency (downtime past the p90 floor; warns only when the player is >1σ **below** the top-parse efficiency - never critical, so beating the top parses never trips it), and success. Plus the **rule engine** and the per-cooldown comparison table. A rule's `condition` is one of twelve kinds, dispatched by `evaluateCondition` over a `RuleContext` that `buildRuleContext` derives once per player view: `cast_without_prior` (honouring its `position` of `before` (default) / `after` / `either`), `hold_cooldown_for_anchor`, `cast_outside_buff`, `aura_uptime_below` (`on: self` reads the buff stream, `on: target` the enemy debuff stream), `opening_sequence`, `cast_at_target_count` (distinct `targetID:targetInstance` damaged within `TARGET_COUNT_WINDOW_S` of the cast), `resource_at_cast` (the pre-cost `classResources` snapshot on the cast), `proc_wasted`, `filler_in_buff` (the share of the filler casts inside a state that went to the coached builder), `spend_at_stacks` (the `stacksAt` count on the cast, so a stacking buff that acts as a resource is judged like one), `aura_clipped` (per-target spans from `buildTargetedAuraSpans`, counting only refreshes within `HARD_CAST_WINDOW_S` of a cast, since most refreshes in a log are procs), and `filler_below_health` (the same share as `filler_in_buff` under the condition's own `health_pct`, read from the last damage event carrying the target's `hitPoints`). The three share-of-fillers and stack kinds take `except_buff_spell_ids`, which drops the windows that suspend the choice. There is deliberately no "keep X on cooldown" kind: `checkLostUses` already flags that for every `major_cooldowns` entry, so a rule would duplicate a row the player already has and could contradict it. The player fetch is `Casts` (with `includeResources`) plus `Buffs` always, and `Debuffs` / `DamageDone` only when `rulesNeed` says a rule reads them - the enemy-aura stream is raid-wide and costs several pages, since WCL answers no narrower shape. The `targetHealth` stream rides on `damage` rather than standing alone: it asks for the heavier `includeResources` form, which is the only shape carrying the struck actor's `hitPoints`. `rulesNeed`, `evaluateCondition`, `ruleLabel`, `ruleApplicable` and `consensusGated` all switch exhaustively over the `RuleCondition` union, so a new kind cannot compile until it declares its streams, its label, its applicability and its gating.

The rule engine is the slice's functional core in `rotation-rules.ts`, separate from `rotation.service.ts` so the ingest transform measures top parses with the same code the runtime judges the player with. **A rulebook rule names only what to look at; every magnitude comes from the encounter.** The transform builds a `RuleContext` per benched parse, `measureRule` returns that pull's contribution (widest lead needed, gap kept clear before an anchor, uptime held, opener completion time, median targets engaged, median resource as a share of its own cap, share of the fillers the coached builder won, median stacks spent at, median duration left at a refresh), and `ruleThreshold` takes the median plus a band of `max(stddev, THRESHOLD_BAND_MIN_FRAC * median)` once `THRESHOLD_SAMPLE_FRAC` of parses could be measured. That lands in the bench as `rules: BenchedRule[]`. At runtime `benchedRules` drops a rule whose kind needs a magnitude the encounter could not supply, and each evaluator reads the band through `lenient`, always shifted to the forgiving side. `cast_outside_buff` and `proc_wasted` need no magnitude, so they never depend on a threshold. `measureRule` and `needsThreshold` switch exhaustively alongside `streamsFor`, `evaluateCondition`, `ruleLabel` and `ruleApplicable`, so a new kind must state where its magnitude comes from. The effect is that an encounter the field plays against the general rule moves the bar rather than failing the player against a static number, which is the same principle as `MIN_USE_SHARE_FRAC` and the measured `bl_pct` gate.
