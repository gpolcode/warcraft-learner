...
The rule engine is the slice's functional core in `rotation-rules.ts`, separate from `rotation.service.ts` so the ingest transform judges top parses with the same code the runtime judges the player with. **Rules are spec-level but consensus is per encounter**: the transform builds a `RuleContext` per benched parse, `judgeRules` records applicable/followed per rule, and `ruleConsensus` aggregates into the bench's `rules: BenchedRule[]` (`followed_pct` over the parses the rule applied to, `null` when none could be judged). At runtime `consensusRules` drops a **gated** rule unless at least `RULE_CONSENSUS_PCT` of applicable top parses followed it, so an encounter the field plays against the general rule is never flagged. Gated: `aura_uptime_below`, `cast_at_target_count`, `opening_sequence`, `hold_cooldown_for_anchor`, `cast_without_prior`, `resource_at_cast` - each carries an authored number or order whose right answer shifts with the fight. Ungated: `cast_outside_buff` and `proc_wasted`, which have no encounter-specific number. This is the same consensus principle as `MIN_USE_SHARE_FRAC` and the measured `bl_pct` gate, applied to rules. Each evaluates into a row whose severity comes from its `priority` (`ruleSeverity`: critical -> critical, high -> warning, medium/low -> info) and whose chip comes from its `type` (`RULE_TYPE_LABEL`). A violated rule and a followed rule both render the rule's `description`, so one rule reads as one rule in either state; `ruleApplicable` gates the followed list so a rule the pull never tested is listed in neither. The schema requires a `condition`, so every rule is judged against the player's recorded behaviour; there is no display-only rule text anywhere in the UI. A rulebook still carrying a null condition is skipped until it is re-authored. Findings split into **Needs Improvement** / **Timing Suggestions** / **Doing Well**.
