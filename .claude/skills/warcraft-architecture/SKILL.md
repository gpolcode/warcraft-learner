...
- **Self-contained services - import ONLY the two API services (or the slice `*DataSource` token) + models + `logWarn`.** Both the `*TransformService` and the `*FeatureService` follow this: no importing of outside analysis, mappers, or UI components. Each **reimplements/owns** its math as named, pure, **total** functions (returns `0`/`null`/`[]` for empty input, never throws; optional findings return `T | null`), **exported from and colocated in that service's own `*.service.ts`**, with no Angular/`inject()`/IO. The one exception is code both of a slice's services need: that goes in a single slice-local `<slice>-<noun>.ts` module with its own colocated spec (`gear-extract.ts`, `map-draw.ts`, `rotation-rules.ts`), never duplicated into each service. Self-containment over sharing: the transform owns its own math and pulls nothing from outside analysis or mappers (ingestion runs this very service, so there is no second implementation to keep aligned). Data shapes a service needs (view-model rows like `ComparisonWindow`/`RangeRow`, ranking rows like `ParseRanking`) live in `core/models`, never in a component or mapper file. Tested directly in `*.service.spec.ts`. Cross-slice **presentational** derivations may still live under `shared/` (e.g. `shared/gear/gear-comparison.ts`); likewise generic, pure, **non-domain** math/formatting primitives (rounding, time clustering, outlier predicates, expected-use arithmetic, clock formatting, severity ordering) are blessed in `shared/analysis/analysis-math.ts`, generic (non-domain) WCL-response projections and window view-row builders in `shared/analysis/wcl-projections.ts` (it owns `toParseRankings` and `windowSpells`), the prior-relative hold-target math in `shared/analysis/hold-targets.ts` (`detectHoldWindows`, `buildHoldTargets`, `holdSuggestionFindings` and their thresholds), which rotation and defensive both consume on identical terms, and `relativeS`/`withRelativeS`/`TimedEvent` in `shared/analysis/wcl-projections.ts` (see the time-unit rule below) - all may be imported by the transform/feature services rather than re-declared per slice, while each slice still owns its own DOMAIN clustering/benchmark math, colocated in its `*.service.ts`. **Never add a separate `*.vm.ts` view-model file** - a page shell's pure helpers (report-code parsing, fight/player projection, auto-select) are colocated and exported from the page's own `*.ts`, e.g. `post-raid.ts`, and tested in its `*.spec.ts`. One carve-out: importing generic statistics primitives from `d3-array` (`mean`/`median`/`deviation`/`quantile`) directly is permitted, the same way `Math` is - they are pure arithmetic, not domain analysis. Call them directly at the use site (guard d3's `undefined`-on-empty return with `?? 0` to preserve the total-function contract); the shared `round` (and the other generic primitives listed above) come from the blessed `shared/analysis/analysis-math.ts` rather than being re-declared per service, since `d3-array` has no rounding function.
- **Seconds is the one time unit analysis code ever sees - stamped once, at the fetch boundary, never re-derived downstream.** WCL event/fight timestamps arrive as report-relative milliseconds (`WclEvent.timestamp`, `WclFight.startTime`/`endTime`) - the wire DTO, kept in ms because `WclApiService` cannot remap it and each slice independently fetches its own events. `shared/analysis/wcl-projections.ts` owns the one conversion: `relativeS(laterMs, earlierMs)` for a single ms delta (a fight duration, a gap between two casts), and `withRelativeS(events, fightStartMs): TimedEvent[]` to stamp a whole event array with `atS` in one pass (`TimedEvent = WclEvent & { atS: number }`). Each `*Service`'s async fetch method calls `withRelativeS` immediately on the arrays `wclApi.getAllEvents` returns; every pure function downstream - the rule engine, the transform/feature service math, `aura-windows.ts` - takes `TimedEvent[]` and reads `.atS` directly. None of them accept a raw `WclEvent[]` alongside a separate `fightStartMs`/`fEndMs` parameter to re-derive relative time internally: that shape is exactly the anti-pattern this rule forbids, since it lets ms leak past the boundary and forces every caller (including unit tests) to keep re-deriving the same relativization. A function that only needs a fight's *length*, not per-event times, takes an already-computed `fightDurationS` rather than a raw ms end boundary. Every field, parameter, and persisted bench value past the boundary is seconds (`_s` suffix - `atS`, `fightDurationS`, `downtime_threshold_s`), never re-scaled inline (no bare `* 1000` / `/ 1000` in analysis code). Test fixtures and builders take seconds directly (`src/testing/builders/events.ts` converts to the wire's ms internally; a spec never writes a ms magic number) and stamp them with `withRelativeS(events, 0)` before calling into a function that expects `TimedEvent[]`. The only other native-ms values are genuine non-WCL boundaries, each documented at its own definition: `live-capture.service.ts`'s wall-clock fields (shares one clock with `Date.now()`/`MediaRecorder`), `wcl-auth.ts`'s token-expiry cache, `LiveReportSyncService.POLL_INTERVAL_MS` (an RxJS `interval()` timer unit, with an independent `POLL_INTERVAL_S` literal kept in sync for display), and `map-canvas.ts`'s `requestAnimationFrame` delta.
...
2. Each `*FeatureService` reads its prepared bench via its `*DataSource` (the tailored file in prod, or the live `*TransformService` in the development and ingest environments) and fetches the player's own log (`Casts`/`Buffs`/`DamageDone`/`DamageTaken`) via the cached `WclApiService`, then computes its slice with its colocated pure functions:
   - **Rotation** (`rotation.service.ts`) - per offensive cooldown: lost casts (`expected` from top-parse `uses_per_min`; the lost-cast and first-cast checks are gated on a use-share majority - `used_sample_count / sample_count >= MIN_USE_SHARE_FRAC` - so a situational cd most top parses skip is never flagged as "unused"), bloodlust alignment (gated by measured `bl_pct >= 50`, not the rulebook flag), first-cast delay (>2σ), held-past-reset (>2σ above `avg_gap_s`, skipped when null), hold suggestions (a majority of top parsers hold past the prior cast + cooldown; flagged when the player's own prior-relative gap falls below the consensus band), cast efficiency (downtime past the p90 floor; warns only when the player is >1σ **below** the top-parse efficiency - never critical, so beating the top parses never trips it), and success. Plus the **rule engine** and the per-cooldown comparison table. A rule's `condition` is one of the twelve kinds tabled under **Rule condition kinds** below, dispatched by `evaluateCondition` over a `RuleContext` that `buildRuleContext` derives once per player view. There is deliberately no "keep X on cooldown" kind: `checkLostUses` already flags that for every `major_cooldowns` entry, so a rule would duplicate a row the player already has and could contradict it.

The rule engine is the slice's functional core in `rotation-rules.ts`, separate from `rotation.service.ts` so the ingest transform measures top parses with the same code the runtime judges the player with. **A rulebook rule names only what to look at; every magnitude comes from the encounter.** The transform builds a `RuleContext` per benched parse, `measureRule` returns that pull's contribution, and `ruleThreshold` takes the median plus a band of `max(stddev, THRESHOLD_BAND_MIN_FRAC * median)` once `THRESHOLD_SAMPLE_FRAC` of parses could be measured. That lands in the bench as `rules: BenchedRule[]`. At runtime `benchedRules` drops a rule whose kind needs a magnitude the encounter could not supply, and each evaluator reads the band through `lenient`, always shifted to the forgiving side. The effect is that an encounter the field plays against the general rule moves the bar rather than failing the player against a static number, which is the same principle as `MIN_USE_SHARE_FRAC` and the measured `bl_pct` gate.

**What each parse contributes depends on how the rule is judged, and the two must agree.** A kind that judges *every* cast benches the parse's **forgiving extreme** - the widest lead it needed, the tightest gap it kept, the cheapest spend it allowed, the earliest it re-applied - because benching a central tendency and then flagging individual casts puts half the field's own casts on the wrong side of its own bar. A kind that judges an *aggregate* benches that same aggregate: a share against a share, an uptime against an uptime.
...
**Every kind also reports its per-instance detail as `occurrences: FindingOccurrence[]` on the finding (`core/models/analysis.models.ts`), so a "3 / 40" summary is never the only thing the player can act on.** This is a hard requirement for a new kind, not an optional enrichment - the finding table's expand chevron and the `wl-finding-occurrences` moment strip (`shared/components/finding-table/`) render only when `occurrences` is populated, so a kind that omits it silently loses the drill-down UX every other kind has. Each occurrence is `{ atS?, ok, label, note?, detail, marker? }`: `atS` is fight-relative and left unformatted (the template pipes it, never the engine - see the all-formatting-through-pipes rule in **warcraft-frontend**); `label` is the short chip value (a lead in seconds, a target count, a buff state, a short ability name); `ok` distinguishes a passing instance from a violating one so the strip shows the whole picture, not just the misses; `note` stands in for a formatted timestamp when `atS` is absent (an opener step that was never reached); `detail` is the full sentence the strip shows for whichever occurrence is selected; `marker` renders a reference instant (the anchor cast a hold-for rule holds against) in a neutral tone rather than as a judged pass/fail. Cap a kind's occurrence list through `sampleOccurrences` (`MAX_OCCURRENCES` in `rotation-rules.ts`) so a busy fight cannot bloat the strip. `aura_uptime_below` additionally reports `timeline: FindingTimeline` (`{ segmentsS, fightDurationS }`, merged up-spans) so its finding renders a fight-length coverage bar instead of a chip-per-cast strip - reach for a second display variant like this only when a kind's data is genuinely continuous-over-time rather than per-instance; every other kind uses the one moment-strip variant. `evaluateBoundedPerCast` builds occurrences generically for the three kinds that share it (`cast_at_target_count`, `resource_at_cast`, `spend_at_stacks`); `fillerOccurrences` does the same for the two filler kinds. A new kind should extend one of those shared builders before writing a bespoke one.
...
| Hold suggestion trigger | Cast index (1-based) where at least the consensus fraction of TOTAL sampled parses delay more than `HOLD_THRESHOLD_S` past the **prior actual cast + cooldown**; fires only when the player's own prior-relative gap is below `delay_s - band_s` (`band_s = max(σ, HOLD_BAND_MIN_S)`). Over-holding is not flagged. Rotation and defensive share one implementation in `shared/analysis/hold-targets.ts`, so `HOLD_CONSENSUS_FRAC` gates both | None emitted when `hold_targets` is empty (no parsers held at that index) |
| Downtime gap floor | `DOWNTIME_PERCENTILE` of pooled `castGapListS` | Always runs when bench exists (`downtime_threshold_s` is required, defaults to `DEFAULT_DOWNTIME_THRESHOLD_S`) |
...
1. Bucket DamageDone into `BIN_S` bins over the fight; take a `ROLL_BINS`-bin forward rolling damage.
