...
- **Every term the tool coins is definable within one interaction.** Game vocabulary is free - `Colossus Smash`, `uptime`, `Bloodlust` need no gloss. A word this app invented does: `top parses`, `hold`, `burst window`, `on plan`, `anchor`, `reference`. Each needs its definition one hover or one click from where it appears - a tooltip on the label, or the shared explainer in `shared/components/benchmark-explainer` that every card subtitle triggers. A coined term ships with that definition or it does not ship.
...
- **A flagged finding never prints its measured value and its target as the same string.** Display rounding stops where the two would collide: a 97.8% uptime under a 98.4% floor reads `97.8 / 98.4`, never `98 / 98`, which tells the reader they hit the mark the row is flagging them for missing.
...
- **The compact `measured` cell keeps the short form, not a private code.** `{ value, unit }` renders in a narrow table column, so `+4s` / `top 0:08` stay. Three limits keep it readable: the unit is real words (`% uptime`, `cast(s)`, `in Bloodlust`), never an abbreviation the reader cannot expand (`in BL`); the value reads your pull first and what it was judged against second, the one convention the benchmark explainer states so every cell can lean on it; and the prose rules above still govern `message`, `remedy` and `occurrenceTarget`.
