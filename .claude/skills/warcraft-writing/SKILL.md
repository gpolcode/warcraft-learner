...
- **Name the comparison group `top raiders` in prose, `top logs` in data labels.** `"Top raiders average 12s."`, `"of top logs"`. Never `"Top:"`, `"the field"`, `"top parses"` or `"top parsers"` on either surface; `parse` names a Warcraft Logs ranking, so it appears only where the benchmark itself is defined.
