...
- **Share card** - `frontend/public/og-image.png` is the same shield over the `warcraft-learner` wordmark set in Schibsted Grotesk, 1200x630 (the 1.91:1 ratio social crawlers want), and is a derivative of the favicon artwork: regenerate it whenever the logo changes. It backs `og:image` / `twitter:image` in both `frontend/src/index.html` and the gh-pages root redirect (`.github/pages-root/index.html`), which carry identical tags because crawlers do not follow that page's meta refresh. Both reference it by absolute URL under the deployed shell (`https://warcraft-learner.com/main/og-image.png`).
