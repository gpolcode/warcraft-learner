...
- **A coined term carries its definition or gets rewritten.** Prefer wording a first-time reader already understands; where a word this app invented has to stay - `hold`, `burst window`, `on plan`, `anchor`, `reference` - the label it sits on carries a tooltip that defines it.
