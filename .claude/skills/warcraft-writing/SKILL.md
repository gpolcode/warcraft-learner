...
- **Name the comparison group by surface: `top raiders` in prose, `top logs` in data labels.** A sentence names the people it is measuring you against (`"Top raiders average 12s."`, not `"top 12s."`); a column label, unit or cell names the data (`"of top logs"`, `"8/10 logs"`). Never a bare `"Top:"` prefix, never `"the field"`, and never `"top parses"` or `"top parsers"` on either surface - all three are internal shorthand that means nothing to a new user. The word `parse` names a Warcraft Logs ranking, so it appears only where the benchmark itself is defined, never as the name of the group.
- **A card subtitle may name the group in full, and each page's first card should.** The terse `"X vs top logs."` is the default; on the first card a reader meets, prefer the expanded `"vs the top Mythic logs for your spec."` so the ruler is named before any number is read.
