...
- **A coined term carries its definition in visible copy, or gets rewritten.** Prefer wording a first-time reader already understands; where a word this app invented has to stay - `hold`, `burst window`, `on plan`, `reference` - a visible line beside it defines it. Never a tooltip: hover text is invisible to exactly the reader who needs it, and absent on touch.
