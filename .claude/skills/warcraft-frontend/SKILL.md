...
`index.html` does not load the Wowhead scripts. `WowheadTooltipsService` (`core/services/wowhead-tooltips.ts`) injects `wh-tooltips-config.js` and then the zamimg `tooltips.js` the first time a Wowhead link renders: `wl-game-icon` (the only component that emits a `wowhead.com` anchor) calls `ensureLoaded()` from an `afterNextRender`, and the service is idempotent so repeated calls inject once. `tooltips.js` installs a DOM observer on load, so that single injection decorates every link rendered afterward and across navigations. This keeps the ~100 kB of config CSS + script off pages with no Wowhead links (the landing page). The service chains `tooltips.js` off the config script's `load` event because dynamically inserted scripts otherwise have no execution-order guarantee, and `tooltips.js` needs the `whTooltips` global the config sets. `afterNextRender` is browser-only, so prerendered pages never pull the zamimg assets.
