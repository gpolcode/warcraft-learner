...
| **The partition decides which parses you see** | Without an explicit partition WCL answers from its default, whose top parses can be months old. Query partitions newest-first (`rankingsFromPartition`) or a current raid reads as dead. |
| **Nothing in the API says which raid is current** | WCL leaves a finished tier unfrozen and ranked, still collecting parses, and a new patch gives it a fresh partition so even its top parses look recent. It also keeps a frozen copy of a zone under the same name, with different encounter ids. The raids to bench are named in the `CURRENT_RAIDS` repo variable rather than inferred. |
