...
| **Solving the talent format problem** | The gear transform derives the `v2:` talent key from each parse's CombatantInfo `talentTree` (the nested `nodeID` dict, via `talentKeyFromTree` in `gear-extract.ts`) - there is no separate `encounterRankings` query. Both the bench and the analyzed player's key come from CombatantInfo, so they are the same `v2:` format and compare directly; the `v1:` `characterRankings` format is not used. |
...
| **`gameData.spell()` was removed** | Spell icons and names must come from `masterData.abilities` in the report response (or `gameData.ability(id)`, which resolves any real id but returns `null` for a nonexistent one - see the ingest rulebook spell-id gate). |
