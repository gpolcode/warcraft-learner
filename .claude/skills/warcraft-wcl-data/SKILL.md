...
| **A ranking's `startTime` is epoch ms; the partition decides which parses you see** | Without an explicit partition WCL answers from its default, whose top parses can be months old. Query partitions newest-first (`rankingsFromPartition`) or a current raid reads as dead. |
| **Parse recency cannot tell a current raid from a finished tier** | WCL leaves a finished tier non-frozen and ranked, and a new patch gives it a fresh partition, so its newest-partition board is all recent parses too. Only the zone ordering separates them: raid zone ids ascend with release, and WCL's own zone list is sorted descending by id. |
