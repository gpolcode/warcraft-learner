...
The browser authenticates to WCL with the **client-credentials** grant against `/api/v2/client`, using a client id + secret **hardcoded in `src/app/domains/raid-analysis/data/http/wcl-public-client.ts`** (read directly by its neighbour `wcl-auth-service.ts` - and therefore shipped, public, in the static JS bundle). This is a deliberate trade-off, not a leak to fix:
...
| **A `death` event names the victim as its target** | A `Deaths` fetch with `sourceID` = the player returns nothing useful: the source is whatever dealt the killing blow, so the player's own death has another actor as source. Fetch it raid-wide (one small page) and filter `targetID === playerId`. `killingAbilityGameID` carries the killing blow. The pull-overview feature reads it to build the pull's death list and, on a wipe, to derive the wipe time. |
...
The on-disk `positions/{enc_id}.json` shape is not duplicated here - read it from `domains/raid-analysis/data/encounter/positioning.models.ts` (written by `MapTransformService.getBench`, consumed by the map feature: `MapFeatureService` state + the `MapDrawService` geometry). The non-obvious part that bites is the **raw WCL units** stored in each row (x/y in hundredths of a yard, facing in milliradians) - the position/facing-unit quirks in the table above explain the conversions the frontend applies.
