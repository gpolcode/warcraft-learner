...
| **A `death` event names the victim as its target** | A `Deaths` fetch with `sourceID` = the player returns nothing useful: the source is whatever dealt the killing blow, so the player's own death has another actor as source. Fetch it raid-wide (one small page) and filter `targetID === playerId`. `killingAbilityGameID` carries the killing blow. The rotation slice reads it to bound dot-uptime measurement to the time the player was alive, and fetches it only when a rulebook has an uptime rule. |
...
| **A keystone run is marked per fight, not per report** | Only a Mythic+ report's dungeon-boss fights carry `keystoneLevel` (with `difficulty: 10`); its trash segments leave it null, like every raid fight. Zone ids change per season, so the run is identified by scanning the fight list (`isKeystoneReport`, post-raid shell). |
