...
| **Melee auto-attack is event ability id `1`** | Physical auto-attacks carry `abilityGameID: 1`, so melee legitimately appears in a burst/defensive damage breakdown. But `gameData.ability(1)` resolves the stale game spell 1 - "Word of Recall (OLD)", icon `trade_engineering` - not melee (a report's `masterData` names id 1 "Melee", with the icon being the player's equipped weapon, so it varies per parse). `abilityIcons` (`shared/analysis/wcl-projections.ts`) relabels the id-1 sentinel to canonical Auto Attack art (`inv_sword_04` / "Melee") so the row does not render as Word of Recall. |
