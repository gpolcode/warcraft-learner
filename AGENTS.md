...
Load the matching skill(s) **before** you start that step. The `warcraft-*` skills are this project's rules; Angular/TypeScript conventions, the layer boundaries, and the UI styling rules are enforced by ESLint (`frontend/eslint.config.js`), not by a skill. Copilot loads a skill automatically once its `description` matches the task, or you can select it explicitly from the `/` menu - either way, load it before starting the row's work, not from memory.
