...
import { PlayerDetailGroups, WclFight, WclPlayer, WclReport } from '../domains/raid-analysis/data/wcl/wcl.models';
...
export function postRaidProviders(wclApi: unknown, prepareMap = vi.fn(() => Promise.resolve())): unknown[] {
  return [
    ...
    { provide: LiveCaptureFeatureService, useValue: { liveEnabled: signal(false), isCapturing: () => false, clear: vi.fn(), prepare: vi.fn(), setStatus: vi.fn(), scheduleNextPollIn: vi.fn(), clipReady: () => false, openClip: vi.fn() } },
