...
export interface MajorCooldownEntry {
  record: SpellRecord;
  /** The recharge of a charged spell, else its cooldown. */
  cooldownS: number;
...
export class CooldownDerivationService {
  ...
  /** Every APL action whose resolved record is a major cooldown the top parses press, in priority order, with its opener slot read from the parses. */
  majorCooldowns(actions: ResolvedAction[], index: AbilityIndex): MajorCooldownEntry[] {
    ...
    for (const action of actions) {
      ...
      const cooldownS = record ? this.abilities.effectiveCooldownS(record) : null;
      if (!record || cooldownS === null || !this.abilities.isMajorCooldown(record) || this.abilities.usageShare(index, record) < MIN_USAGE_SHARE) continue;
      const entry = byId.get(record.id) ?? { record, cooldownS, lines: [], talentGated: record.talent !== null, openerPriority: null };
  ...
  defensives(index: AbilityIndex, aplTokens: Set<string>): DefensiveEntry[] {
    ...
    for (const [token, records] of index.byToken) {
      if (aplTokens.has(token)) continue;
      const entry = this.defensiveOf(index, token, records);
      if (entry) entries.push(entry);
    }
    return entries.sort((a, b) => a.cooldownS - b.cooldownS);
  }

  private defensiveOf(index: AbilityIndex, token: string, records: SpellRecord[]): DefensiveEntry | null {
    const record = this.abilities.cast(index, token);
    const cooldownS = record ? this.abilities.effectiveCooldownS(record) : null;
    if (!record || cooldownS === null || cooldownS < DEFENSIVE_MIN_COOLDOWN_S || !records.includes(record)) return null;
    // The talent node granting a spell is a passive twin under the same name, so the other-spec check reads every record.
    if (records.some(candidate => this.abilities.talentOfAnotherSpec(candidate, index.specLabel))) return null;
    const effect = record.effects.find(candidate => this.defensiveEffect(candidate));
    return effect?.subtype ? { record, cooldownS, effect: { subtype: effect.subtype, baseValue: effect.baseValue } } : null;
  }
  ...
  private defensiveEffect(effect: SpellEffect): boolean {
    ...
    // A raid-wide health cooldown targets the raid, not the caster, and is still the player's own defensive.
