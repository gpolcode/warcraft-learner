...
import { spellRecord } from '../../../../../testing/builders/simc';
import { EXECUTE } from '../../../../../testing/spell-ids';
...
const record = spellRecord({ id: EXECUTE, name: 'Execute' });
...
function line(gate: string, carries: boolean, priority = 0): ActionLine {
  const own = expressions.parse(gate);
  ...
  return { resolved: { action: 'execute', own, context: [], priority }, record, terms, lineFacts, ...gates.talentGate([...lineFacts.values()]) };
}
...
describe('RuleGateService.consistent', () => {
  const term = (source: string): Fact[] => literals.facts((expressions.termsOf(expressions.parse(source), []) ?? [])[0] ?? []);
...
describe('RuleGateService.termGate', () => {
  it('keeps only the talents every term shares', () => {
    const terms = expressions.termsOf(expressions.parse('talent.a&talent.b|talent.a&!talent.c'), []) ?? [];
