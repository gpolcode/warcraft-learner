...
type AuraField = 'up' | 'stack' | 'refreshable' | 'remains' | 'other';
...
const RESOURCE_FIELDS: Record<string, ResourceField> = { pct: 'pct', deficit: 'deficit', max: 'max' };
...
export class AplLiteralService {
  ...
  private resourceAtom(name: string, second: string, op: AplComparisonOp | null, value: number | null, negated: boolean): Read {
    const field = RESOURCE_FIELDS[second] ?? 'amount';
