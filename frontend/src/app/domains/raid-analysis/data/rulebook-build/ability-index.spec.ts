...
import { builder, finisher, selfAura, spellRecord } from '../../../../../testing/builders/simc';
...
const KIT = [
  ...
  spellRecord({ id: DISPATCH, name: 'Dispatch', className: `Outlaw ${CLASS}`, powerTypes: [ENERGY_TYPE] }),
...
describe('AbilityIndexService.isMajorCooldown', () => {
  it('counts a charge recharge at the floor and not one just under it', () => {
    const atFloor = spellRecord({ id: SHADOW_DANCE, name: 'Shadow Dance', gcd: false, rechargeS: MAJOR_FLOOR_S });
    const under = spellRecord({ id: SHADOW_DANCE, name: 'Shadow Dance', gcd: false, rechargeS: MAJOR_FLOOR_S - 1 });
...
describe('AbilityIndexService.isFiller', () => {
  it('needs a cost and a cooldown under the ceiling', () => {
    expect(abilities.isFiller(builder(BACKSTAB, 'Backstab', { cooldownS: FILLER_CEILING_S - 1 }))).toBe(true);
    expect(abilities.isFiller(builder(BACKSTAB, 'Backstab', { cooldownS: FILLER_CEILING_S }))).toBe(false);
    expect(abilities.isFiller(spellRecord({ id: SHADOW_DANCE, name: 'Shadow Dance', cooldownS: 2 }))).toBe(false);
  });
});

describe('AbilityIndexService.sameRole', () => {
  it('pairs two builders and separates a builder from a finisher', () => {
    expect(abilities.sameRole(builder(BACKSTAB, 'Backstab'), builder(GLOOMBLADE, 'Gloomblade'))).toBe(true);
    expect(abilities.sameRole(builder(BACKSTAB, 'Backstab'), finisher(EVISCERATE, 'Eviscerate'))).toBe(false);
  });
});
