...
import type { AplComparisonOp, AplLiteral, ResolvedAction, SpellRecord } from '../simc/simc.models';
...
import type { AbilityIndex, ParseSample, RuleDraft, RulebookGap } from './rulebook-build.models';
...
export class RuleDerivationService {
  ...
  private fallbackTerm(resolved: ResolvedAction): AplLiteral[] {
    return [
  ...
  private unionLists(base: RuleCondition, extra: RuleCondition): RuleCondition {
    if (base.kind === 'hold_cooldown_for_anchor' && extra.kind === 'hold_cooldown_for_anchor') {
      const spells = this.union({ ids: base.spell_ids, names: base.spell_names }, { ids: extra.spell_ids, names: extra.spell_names });
      return { ...base, spell_ids: spells.ids, spell_names: spells.names };
    }
    if (base.kind === 'proc_wasted' && extra.kind === 'proc_wasted') {
      const spends = this.union({ ids: base.spend_spell_ids, names: base.spend_spell_names }, { ids: extra.spend_spell_ids, names: extra.spend_spell_names });
      return { ...base, spend_spell_ids: spends.ids, spend_spell_names: spends.names };
    }
  ...
  private unionFiller(base: Extract<RuleCondition, { kind: 'filler_in_buff' }>, extra: Extract<RuleCondition, { kind: 'filler_in_buff' }>): RuleCondition {
    const alternatives = this.union(
      { ids: base.alternative_spell_ids, names: base.alternative_spell_names }, { ids: extra.alternative_spell_ids, names: extra.alternative_spell_names });
    const except = this.union(
      { ids: base.except_buff_spell_ids ?? [], names: base.except_buff_spell_names ?? [] }, { ids: extra.except_buff_spell_ids ?? [], names: extra.except_buff_spell_names ?? [] });
    return {
      ...base,
      alternative_spell_ids: alternatives.ids, alternative_spell_names: alternatives.names,
      except_buff_spell_ids: except.ids, except_buff_spell_names: except.names,
    };
  }

  private union(base: NamedIds, extra: NamedIds): NamedIds {
    const ids = [...base.ids];
    const names = [...base.names];
    extra.ids.forEach((id, position) => {
      if (ids.includes(id)) return;
      ids.push(id);
      names.push(extra.names[position] ?? '');
    });
    return { ids, names };
  }
}

/** A condition's parallel id and name lists, merged as one. */
interface NamedIds {
  ids: number[];
  names: string[];
}
