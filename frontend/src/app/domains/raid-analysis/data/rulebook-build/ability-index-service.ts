...
/** Shadow Dance comes back in 20s and is the spec's core window, so the floor sits at it. */
...
export class AbilityIndexService {
  ...
  sameRole(a: SpellRecord, b: SpellRecord): boolean {
    const pools = (record: SpellRecord) => [...new Set(record.powerTypes)].sort((x, y) => x - y).join(',');
  ...
  protected observe(samples: ParseSample[]): ParseObservation {
  ...
  castable(record: SpellRecord): boolean {
    ...
    return record.powerTypes.length > 0 || record.cooldownS !== null || record.rechargeS !== null || record.gcd || record.castTimeS !== null;
  }
  ...
  isFiller(record: SpellRecord): boolean {
    return this.castable(record) && record.powerTypes.length > 0 && (this.effectiveCooldownS(record) ?? 0) < FILLER_MAX_COOLDOWN_S;
  }
  ...
  aura(index: AbilityIndex, token: string, scope: 'self' | 'target'): SpellRecord | null {
    ...
    // A class module tracks a periodic effect apart from its cast as `<spell>_dot`; the dump records it under the spell's name.
