...
import { AplLiteralService, CLEAVE, SINGLE_TARGET, type Atom, type Fact } from './apl-literal-service';
...
export class RuleStateDerivationService {
  ...
  clipRules(groups: ActionGroup[], aura: AuraLookup): DraftSeed[] {
    ...
    for (const group of groups) {
      // A class module tracks a periodic effect apart from its cast as `<spell>_dot`.
  ...
  /** Readiness gates pair a cast with the cooldown it waits for; a far-away gate holds it for that cooldown instead, and a held pair is not also a pairing. */
  cooldownPairingRules(groups: ActionGroup[], index: AbilityIndex): DraftSeed[] {
    const seeds = groups.filter(entry => !this.abilities.isFiller(entry.record)).flatMap(group =>
      this.gates.factSites(group.lines).flatMap(site => {
        const seed = this.pairingSeed(group, site, index);
        return seed ? [seed] : [];
      }));
    const held = new Set(seeds.flatMap(seed => {
      const condition = seed.condition;
      return condition.kind === 'hold_cooldown_for_anchor' ? condition.spell_ids.map(id => `${id}:${condition.anchor_spell_id}`) : [];
    }));
    return seeds.filter(seed => seed.condition.kind !== 'cast_without_prior' || !held.has(`${seed.condition.spell_id}:${seed.condition.required_spell_id}`));
  }
