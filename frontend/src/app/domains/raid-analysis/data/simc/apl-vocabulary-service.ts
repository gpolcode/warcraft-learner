...
import {
  ACTION_OPTIONS, ACTION_RELATIVE_FIELDS, ACTION_WORDS, EXPRESSION_SHAPES, TOKEN_ALIASES, UNRECORDED_NAMES, VARIABLE_OPS, type AplVocabularyEntry,
} from './apl-vocabulary';
...
export class AplVocabularyService {
  ...
  /** The family a bare field of an action's own gate belongs to, null for a field that is not action-relative. */
  actionRelative(field: string): 'dot' | 'cooldown' | null {
    return ACTION_RELATIVE_FIELDS[field] ?? null;
  }

  /** A bare action-relative field answers with its family's row, so `refreshable` alone reads as `dot.*.refreshable`. */
  expression(path: readonly string[]): AplVocabularyEntry | null {
    const [field = ''] = path;
    const family = path.length === 1 ? this.actionRelative(field) : null;
    const shape = family ? `${family}.*.${field}` : this.shape(path);
