...
import type { SpellEffect, SpellRecord, SpellTalentEntry } from './simc.models';
...
export class SpellDataDumpService {
  ...
  private emptyRecord(id: number, name: string, flags: string): SpellRecord {
    return {
      ...
      powerTypes: [], gcd: false, castTimeS: null, executeHealthPct: null, effects: [],
    };
  }
  ...
  private applyField(record: SpellRecord, field: string, value: string): void {
    ...
    switch (field) {
      ...
      case 'Resource': { const powerType = this.powerTypeOf(value); if (powerType !== null) record.powerTypes.push(powerType); break; }
  ...
  /** `35 Energy (3) (id=148)` and `1 - 5 Combo Points (4)`: the first parenthesised number is the WCL power type. */
  private powerTypeOf(value: string): number | null {
