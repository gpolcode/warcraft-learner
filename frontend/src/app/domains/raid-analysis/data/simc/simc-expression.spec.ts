...
const parsed = (source: string): AplExpr => {
  const expr = expressions.parse(source);
  if (!expr) throw new Error(`${source} did not parse`);
  return expr;
};
const roundTrip = (source: string): string => expressions.print(parsed(source));
const simplified = (source: string, resolve: AplResolver = keepEverything): string =>
  expressions.print(expressions.simplify(parsed(source), resolve));
const literals = (source: string): string[][] =>
  (expressions['dnf'](parsed(source), 'own') ?? []).map(term =>
...
describe('SimcExpressionService.parse', () => {
  ...
  it('reads a dotted reference with a numeric segment as one name', () => {
    expect(parsed('trinket.1.has_use_buff')).toEqual({ kind: 'ref', path: ['trinket', '1', 'has_use_buff'] });
  });
  ...
  it('reads nothing from an unbalanced parenthesis, a stray character or a function the grammar lacks', () => {
    expect(expressions.parse('(a&b')).toBeNull();
    expect(expressions.parse('buff.a.remains~=1')).toBeNull();
    expect(expressions.parse('round(energy)')).toBeNull();
  });
});
...
describe('SimcExpressionService.dnf', () => {
  ...
  it('gives a constant-true gate one empty term and a constant-false gate none', () => {
    expect(expressions['dnf'](TRUE, 'own')).toEqual([[]]);
    expect(expressions['dnf'](FALSE, 'own')).toEqual([]);
  });
  ...
  it('expands a gate at the term cap and gives up one pair past it', () => {
    ...
    expect(expressions['dnf'](parsed(pairs(TERM_CAP_PAIRS)), 'own')).toHaveLength(2 ** TERM_CAP_PAIRS);
    expect(expressions['dnf'](parsed(pairs(TERM_CAP_PAIRS + 1)), 'own')).toBeNull();
  });
});
...
describe('SimcExpressionService.termsOf', () => {
  it('crosses the own gate with every inherited gate and tags each literal with its origin', () => {
    const own = parsed('buff.a.up');
    const context = parsed('combo_points>=5|buff.b.up');
