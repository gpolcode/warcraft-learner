...
import { WclReport } from '../wcl/wcl.models';
...
const ERROR_ID = 'feature.player-view';
...
const ANALYSIS = {
  logSource: 'Feature.loadPlayerView',
  errorId: ERROR_ID,
  emptyView: () => EMPTY_VIEW,
  analyze: async (_context: PullContext) => ANALYZED_VIEW,
};

function viewAnalysis(over: Partial<typeof ANALYSIS> = {}): typeof ANALYSIS {
  return { ...ANALYSIS, ...over };
}
...
describe('analyzePull', () => {
  it('hands the analysis the report, the named fight, and the fight duration in seconds', async () => {
    ...
    const result = await pullContext.analyzePull(wclFake(), PULL, viewAnalysis({
  ...
  it('returns the empty view when the fight is not in the report yet, without running the analysis', async () => {
    const UNLOGGED_FIGHT_ID = 99;
    let analyzed = false;
    const result = await pullContext.analyzePull(wclFake(), { ...PULL, fightId: UNLOGGED_FIGHT_ID }, viewAnalysis({
  ...
  it('surfaces a WCL outage as transient, so the feature reports an outage rather than a repro id', async () => {
    ...
    expect(await pullContext.analyzePull(wcl, PULL, viewAnalysis())).toEqual(Results.transient('WCL is unreachable right now.'));
  });

  it('tags an unclassified report failure with the feature repro id instead of an empty view', async () => {
    const wcl = wclFake(async () => { throw new Error('WCL exploded'); });
    const result = await pullContext.analyzePull(wcl, PULL, viewAnalysis());
