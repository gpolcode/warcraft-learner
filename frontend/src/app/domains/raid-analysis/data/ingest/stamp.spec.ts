...
let WITHOUT_PRIVATE = '';
beforeAll(async () => {
  SIGNATURE = await signatures.encounterSkipKey(ROWS, new Set(), VERSION, TOP_N);
  WITHOUT_PRIVATE = await signatures.encounterSkipKey(ROWS, new Set([PRIVATE_PARSE]), VERSION, TOP_N);
});
