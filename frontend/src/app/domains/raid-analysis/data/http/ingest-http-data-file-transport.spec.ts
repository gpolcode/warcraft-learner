...
import { INGEST_VERSION } from '../ingest/ingest-version';
...
const BENCH_BODY = { encounter_id: 3176, sample_count: 5, ingest_version: INGEST_VERSION };
...
describe('IngestHttpDataFileTransport', () => {
  ...
  it('GETs the server-rooted file path and returns Results.ok(body)', async () => {
    ...
    const pending = transport.readJson<typeof BENCH_BODY>(REL_PATH);
    const req = httpMock.expectOne(`${INGEST_SERVER_URL}/api/data/${SERVER_FILE_PATH}`);
    expect(req.request.method).toBe('GET');
    req.flush(BENCH_BODY);

    expect(await pending).toEqual(Results.ok(BENCH_BODY));
  });
  ...
  it('PUTs writes to the file path with the document as the body', async () => {
    ...
    const pending = transport.writeJson(REL_PATH, BENCH_BODY);
    const req = httpMock.expectOne(`${INGEST_SERVER_URL}/api/data/${SERVER_FILE_PATH}`);
    expect(req.request.method).toBe('PUT');
    expect(req.request.body).toEqual(BENCH_BODY);
