/**
 * Statistics primitives and the shared cluster-stats helper for the burst/bench
 * pipeline.
 *
 * Pure functions only (no network, no filesystem). The guarded delegators to
 * simple-statistics preserve the zero-on-empty / zero-on-single contract the call
 * sites rely on (ss.mean/ss.median throw on empty input; sampleStandardDeviation
 * needs n >= 2). `stdev` uses the sample (n-1) standard deviation.
 */

import * as ss from 'simple-statistics';
import { MEMBER_MAJORITY_FRAC } from './thresholds';
import type { ClusterBaseStats } from './models';
