/**
 * Burst window detection and clustering for the burst/bench pipeline (pure).
 *
 * Per-parse: build candidate windows from CD cast times x CD durations, merge
 * near-adjacent ones, and keep those above a damage-share threshold. Across parses:
 * cluster windows by time and surface the common CDs/abilities. Shared by ingest
 * and the browser burst transform.
 */

import { mean, round, groupByTime, clusterBaseStats } from './stats';
import { CLUSTER_MIN_FRAC, MEMBER_MAJORITY_FRAC } from './thresholds';
import type { RulebookCooldown } from '../../models/rulebook.models';
import type { BenchEvent, CdCastSummary, RawBurstWindow, RawBurstWindowAbility, ClusteredBurstWindow } from './models';

export function findBurstWindows(
  damageEvents: BenchEvent[], fightStartMs: number,
  cdSummary: CdCastSummary[], specCds: RulebookCooldown[],
  minPctThreshold = 0.03, castEvents: BenchEvent[] = [],
