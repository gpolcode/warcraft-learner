...
describe('rule engine', () => {
  it('evaluateRules names a violated rule by its description, matching how rulesFollowed names it', () => {
    ...
    const violated = engine.evaluateRules([benched(rule)], ruleCtx([cast(SECRET_TECHNIQUE, 10)]));
    assert.exists(violated[0]);
    expect(violated[0].label).toBe(description);
    expect(engine.rulesFollowed([benched(rule)], ruleCtx([cast(SHADOW_DANCE, 8), cast(SECRET_TECHNIQUE, 10)]))).toEqual([description]);
  });
...
describe('rule severity', () => {
  it.each(['critical', 'warning', 'info'] as RuleSeverity[])('carries an authored %s onto the finding', severity => {
    ...
    const finding = engine.evaluateRules([benched(rule)], ruleCtx([cast(SECRET_TECHNIQUE, 10)]))[0];
...
describe('ruleLabel', () => {
  it('prefers the rule description when present', () => {
    expect(engine['ruleLabel'](SECRET_TECH_NEEDS_DANCE, 'Pair Shadow Dance with Secret Technique'))
...
describe('rulesFollowed', () => {
  ...
  it('lists the rule when Shadow Dance is paired with Secret Technique', () => {
    expect(engine.rulesFollowed([benched(pairDanceWithSecretTech)], ruleCtx([cast(SHADOW_DANCE, 10), cast(SECRET_TECHNIQUE, 12)])))
      .toEqual(['Pair Shadow Dance with Secret Technique']);
  });

  it('omits the rule when Shadow Dance is cast without Secret Technique', () => {
    expect(engine.rulesFollowed([benched(pairDanceWithSecretTech)], ruleCtx([cast(SHADOW_DANCE, 10), cast(SECRET_TECHNIQUE, 30)]))).toEqual([]);
  });

  it('omits the rule when Secret Technique was never cast', () => {
    expect(engine.rulesFollowed([benched(pairDanceWithSecretTech)], ruleCtx([cast(SHADOW_DANCE, 12)]))).toEqual([]);
  });
  ...
  it('lists the rule when Shadow Dance is held clear of Shadow Blades', () => {
    ...
    expect(engine.rulesFollowed([benched(holdDanceForBlades, wideCeiling)], ruleCtx([cast(SHADOW_BLADES, 10), cast(SHADOW_BLADES, 120), cast(SHADOW_DANCE, 50)])))
  ...
  it('omits the rule when Shadow Dance is spent in the hold window before Shadow Blades', () => {
    expect(engine.rulesFollowed([benched(holdDanceForBlades, band(HOLD_WINDOW_S))], ruleCtx([cast(SHADOW_BLADES, 10), cast(SHADOW_BLADES, 120), cast(SHADOW_DANCE, 110)]))).toEqual([]);
  });

  it('omits the rule when the held cooldown was never cast', () => {
    expect(engine.rulesFollowed([benched(holdDanceForBlades, band(HOLD_WINDOW_S))], ruleCtx([cast(SHADOW_BLADES, 10), cast(SHADOW_BLADES, 120)]))).toEqual([]);
  });
...
describe('ruleBand', () => {
  ...
  it('pools every instance across parses into one field, not each parse\'s own median', () => {
    ...
    const result = engine.ruleBand(instanceCond, perParse);
  ...
  it('counts sample_count as pooled instances, for an instance-pooled kind', () => {
    ...
    expect(engine.ruleBand(instanceCond, perParse).sample_count).toBe(25);
  });

  it('leaves a parse that measured nothing out of the pool', () => {
    const result = engine.ruleBand(parseCond, samples([[1], [2], [3], [], [4], [5]]));
  ...
  it('reads a pool at MIN_POOLED_INSTANCES off its percentiles, and a thinner one off its own min and max', () => {
    ...
    const thickBand = engine.ruleBand(instanceCond, thick).band;
    ...
    expect(engine.ruleBand(instanceCond, thin).band).toMatchObject({ lo: 1, hi: 10 });
  });

  it('bars a band below MIN_MEASURED_PARSES', () => {
    expect(engine.ruleBand(parseCond, samples([[1], [2], [3], [4]])).band).toBeNull();
  ...
  it('keeps a two-sided rule whose near edge is degenerate, since its far edge still judges', () => {
    ...
    expect(engine.ruleBand(instanceCond, perParse).band).toMatchObject({ lo: 0, hi: 0 });
  });
  ...
  it('drops a two-sided rule only once BOTH edges are degenerate', () => {
    ...
    expect(engine.ruleBand(instanceCond, perParse).band).toBeNull();
  });
  ...
  it('drops a one-sided rule whose only edge lands on the domain maximum, since no cast can clear it', () => {
    ...
    expect(engine.ruleBand(genAtCap, perParse).band).toBeNull();
  });
  ...
  it('returns a null band once the field disagrees with its own edge past MAX_TOLERANCE', () => {
    ...
    const result = engine.ruleBand(instanceCond, samples([bad, bad, good, good, good, good, good, good, good, good]));
  ...
  it('measures tolerance as the p90 of each contributing parse\'s own out-of-band share', () => {
    ...
    const result = engine.ruleBand(instanceCond, samples([oneOffParse, cleanParse, cleanParse, cleanParse, cleanParse]));
  ...
  it('counts an instance the parse judged but could not measure as out of band, so tolerance defends the population the runtime judges', () => {
    ...
    expect(engine.ruleBand(pairedCond, perParse).band?.tolerance).toBeCloseTo(0.25, 5);
    expect(engine.ruleBand(pairedCond, measuredOnly).band?.tolerance).toBe(0);
  });
  ...
  it('pins tolerance at exactly 0 for a parse-pooled kind, whose parses each contribute one value', () => {
    ...
    expect(engine.ruleBand(wasted, perParse).band?.tolerance).toBe(0);
  });
  ...
  it('collapses lo and hi to the same value when every parse agrees exactly', () => {
    ...
    expect(engine.ruleBand(blackPowder, perParse).band).toEqual({
...
describe('benchedRules', () => {
  ...
  it('keeps a rule whose band this encounter measured', () => {
    expect(engine.benchedRules([benched(needsMagnitude)]).map(entry => entry.rule)).toEqual([needsMagnitude]);
  });

  it('drops a rule whose band the encounter could not measure, rather than inventing one', () => {
    expect(engine.benchedRules([benched(needsMagnitude, null)])).toEqual([]);
  });
  ...
  it('drops a row still carrying the shape a residual deployed file bakes, since entry.band reads undefined on it', () => {
    ...
    expect(engine.benchedRules([stale])).toEqual([]);
  });
});
...
describe('rulesNeed', () => {
  ...
  it.each(cases)('$name', ({ rules, stream, needed }) => {
    expect(engine.rulesNeed(rules, stream)).toBe(needed);
  });
});
