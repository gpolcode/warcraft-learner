...
export class IngestStampService {
  private readonly signatures = inject(IngestSignatureService);

  /** `source_signature` drives the skip check; the bare `ingest_version` drives the work-ordering. */
  stampSignature<T extends object>(data: T, signature: string, stamp: IngestStamp): T & StampedFile {
  ...
  private readStamp(file: unknown): StoredStamp {
  ...
  readFileStamp(file: unknown): { version: number | null; ingestedAtS: number | null } {
    const { version, ingestedAtS } = this.readStamp(file);
