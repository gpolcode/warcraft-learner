...
const holdTargetsSvc = TestBed.inject(HoldTargetsService);
...
describe('detectHoldWindows', () => {
  ...
  it('flags a cast more than 8s past the prior cast + cooldown', () => {
    ...
    const holds = holdTargetsSvc.detectHoldWindows([0, 110], EFFECTIVE_CD_S);
  ...
  it('does not flag a cast exactly at the threshold (strict)', () => {
    ...
    expect(holdTargetsSvc.detectHoldWindows([0, 98], EFFECTIVE_CD_S)).toHaveLength(0);
    expect(holdTargetsSvc.detectHoldWindows([0, 98.1], EFFECTIVE_CD_S)).toHaveLength(1);
  });
  ...
  it('measures each hold from the prior cast, so one hold does not cascade', () => {
    ...
    const holds = holdTargetsSvc.detectHoldWindows([0, 200, 290], EFFECTIVE_CD_S);
    expect(holds).toHaveLength(1);
  ...
  it('returns nothing with a single cast', () => {
    expect(holdTargetsSvc.detectHoldWindows([5], EFFECTIVE_CD_S)).toEqual([]);
  });
});
...
describe('buildHoldTargets', () => {
  ...
  it('surfaces a target when a majority of parses hold at that index', () => {
    ...
    const targets = holdTargetsSvc.buildHoldTargets([...heldAt(110, 20, 5), ...noHolds(5)], EFFECTIVE_CD_S);
  ...
  it('drops a target below the majority (strict boundary)', () => {
    ...
    const targets = holdTargetsSvc.buildHoldTargets([...heldAt(110, 20, 4), ...noHolds(6)], EFFECTIVE_CD_S);
  ...
  it('records prior-relative delay, absolute target, and a band floored at 5s', () => {
    ...
    const targets = holdTargetsSvc.buildHoldTargets(heldAt(110, 20, 3), EFFECTIVE_CD_S);
  ...
  it('bands on the delay spread when it clears the floor', () => {
    ...
    expect(holdTargetsSvc.buildHoldTargets(spread, EFFECTIVE_CD_S)).toMatchObject({
  ...
  it('keys consensus on the sample size passed, not the holders alone', () => {
    ...
    expect(holdTargetsSvc.buildHoldTargets(heldAt(110, 20, 2), EFFECTIVE_CD_S, TOTAL_PARSES)).toEqual({});
  });
});
