...
export class WindowViewService {
  ...
  private eventDamage(event: TimedEvent): number {
  ...
  private windowBreakdown(
    ...
    const winTotal = winEvents.reduce((sum, event) => sum + this.eventDamage(event), 0);
    ...
    for (const event of winEvents) {
      ...
      const spellId = this.projections.normalizeAbilityId(event.abilityGameID);
      byAbility[spellId] = (byAbility[spellId] ?? 0) + this.eventDamage(event);
    }
    const attribution = options.attribution;
    const counts = attribution ? this.castsByName(casts, inWindow, attribution.nameOf) : new Map<string, number>();
  ...
  playerWindowDamage(
    ...
    const sorted = damageEvents
      .filter(event => event.atS >= 0 && this.eventDamage(event) > 0)
      .sort((a, b) => a.atS - b.atS);
    const casts = options.attribution?.casts.filter(event => event.type === 'cast' && event.abilityGameID) ?? [];
    return topWindows.map(window => this.windowBreakdown(window, sorted, casts, options));
  }

  private windowDetailRows(
    ...
    return abilityBreakdown.map(ability => {
      const baked = this.projections.resolveAbility(abilities, ability.spell_id, 'windowDetailRows');
  ...
  buildWindowView<A>(
    ...
    topWindows.forEach((window, index) => {
      ...
      windows.push({
        ...
        detailRows: this.windowDetailRows(window.ability_breakdown, playerWindow, abilities, adapter.castColumns ?? false),
      });
