...
Behavior is implemented as methods on `@Injectable` services - stateless, data in, data out (eslint-enforced); IO lives only under `data/http` (`WclApiService`, `DataFileApiService`, `SimcDataService` and the static-data readers beside them). Ingestion is the same Angular app booted with the `ingest` configuration (`feature-ingest`), driving the same `*TransformService`s and persisting through a micro file server to `frontend/public/data/specs/**`. Each encounter's rulebook is derived in memory during ingest, deterministically, from SimulationCraft's action priority list and spell data plus the encounter's own top parses (`data/simc`, `data/rulebook-build`), and is baked into the benches. The development configuration derives it the same way for the analysis on screen, so `npm start` needs no ingested data. Every SimulationCraft token and name the builder understands is inventoried in `data/simc/apl-vocabulary.ts` with what it maps to or why it is dropped; one outside the inventory is reported by the ingest run as a gap. A person hides a name from a spec's derivation through the removal-only overlay in `data/rulebook-build/rulebook-exclusions.ts`. A spec SimulationCraft writes no action priority list for gets gear, positions and burst windows only.
...
| Touching the SimulationCraft reading (`data/simc`, `data/rulebook-build`), the inventory or the exclusion overlay, or closing a SimulationCraft gap issue | **warcraft-simc** |
