...
- **Bench-derived fields are required, never optional, and read directly - no defaulting at the use site.** A new field a finding/view-model derives from is typed as **required** (`is_passive: boolean`, `topCasts: number`), not optional (`is_passive?`, `topCasts?: number | null`). Because the data is complete, read it straight: `row().passive`, never `row().passive ?? false` or `=== true`; `ability.avg_casts`, never `ability.avg_casts ?? 0`. Do **not** write a `?? <default>` for it in a `computed()` **or a template** (`{{ x ?? 0 }}`, `{{ y ?? '-' }}`), and do **not** add a null-guard (`if (x == null) return 'muted'`). Provide the real value at the single construction site. The genuinely-nullable bench fields are the documented few (`avg_gap_s`, `avg_bl_offset_s`, etc. - see "Analysis thresholds"); everything else is required. When a field is structurally irrelevant to a **different slice that shares the same type** (e.g. the burst casts fields `avg_casts`/`is_passive`/`playerCasts`/`topCasts`/`passive` on a defensive ability row or a window `overview` summary, which never render a casts column), set an **explicit neutral value** (`0` / `false`) at that slice's construction site - do **not** make the field optional to paper over it.
