...
│   ├── ingest-parses.yml        # Hourly via an external cron-job.org workflow_dispatch trigger (GitHub's own schedule cron was too unreliable; see .github/INGEST_TRIGGER.md) + manual: runs `npm run scrape` then `npm run ingest`, commits data/specs/**
