...
- **Comment the why, not the what - one terse line, or none.** A comment earns its place only by stating what the code cannot: a reason, a constraint, a non-obvious consequence. Never narrate what the next line plainly does, and never let a comment run to a paragraph - if it needs one, cut it down. Default to no comment; the reader can read the code.
