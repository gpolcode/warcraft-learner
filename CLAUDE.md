...
- **Never use em-dashes (U+2014), en-dashes (U+2013), the Unicode minus (U+2212), or the middle dot (U+00B7)** anywhere - not in docs, code comments, commit messages, UI copy, or generated output. Use a plain ASCII hyphen (`-`) for ranges and parenthetical asides, or rephrase. This applies to every file in the repo and any text the tooling emits. The `frontend` ESLint config enforces this (`local/banned-characters`) across `.ts`, `.js`/`.mjs`, and `.html`; docs, commit messages, and anything outside that glob still rely on this instruction.
